...
if [[ "$oidc_status" == "0" && "$workload_status" == "0" ]]; then
...
jq -S -n \
  --arg contractStatus "$contract_status" --argjson oidcExit "$oidc_status" --argjson workloadExit "$workload_status" \
  '{gate:"P3-405",status:"SKIPPED",contractChecks:{status:$contractStatus,oidcJwksRotation:{exitCode:$oidcExit},workloadSecretFailClosed:{exitCode:$workloadExit}},liveChecks:{status:"SKIPPED",reason:"The local workload issuer generates one ephemeral signing key at startup and has no hot key/client-secret reload protocol; the four-owner supervisor also does not reload owner credentials in place."},retry:"Add a reloadable issuer with overlapping JWKS keys plus owner credential reload hooks, then rerun RECORD_HUB_P3_ROTATION_LIVE=1 ./scripts/verify-p3-credential-rotation.sh"}' \
