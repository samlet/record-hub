...
| P6-502 | Ops | observation/reconciliation dashboard | [observation contract](phase-6-observation-dashboard.json)；bounded findingCount/recoveryAge/stale signals、metrics 与 Operations console 已接入，仍缺 native recovery/live audit evidence | BLOCKED_BY_OBSERVABILITY_GAP |
| P6-503 | Ops | operator runbook and rollback | [operator runbook contract](phase-6-operator-runbook.json)；静态 procedure 与 `make p6-503-dry-run` 安全证据预检已固化，缺 native rollback drill evidence | BLOCKED_BY_LIVE_ROLLBACK_EVIDENCE |
...
| P6-601 | 数据层 | archive/retention/restore extension | [retention/restore contract](phase-6-retention-restore.json)；`make p6-601-dry-run` 已固化 retention/PITR/restore/rebuild 安全计划预检，缺隔离 live restore evidence | BLOCKED_BY_LIVE_RESTORE_EVIDENCE |
| P6-602 | 架构 | multi-region decision record | [multi-region decision](phase-6-multi-region.json)；`make p6-602-dry-run` 已固化 single-region/evidence/RPO-RTO/边界预检，缺 capacity/RPO/RTO/cost/signoff live evidence | BLOCKED_BY_LIVE_MULTI_REGION_EVIDENCE |
